...
async def fetch_last_price_async(ticker, session, retries=3):
    ...
    for attempt in range(retries):
        if pause_for_403:
            print("Pausing for 1 minute due to 403 error...")
            ...
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    if last_price:
                        ...
                        existing_entry = next(
                            (item for item in price_data if item["symbol"] == ticker),
                            None,
                        )

                        if existing_entry:
                            existing_entry["price"] = round(
                                float(last_price.group(1)), 2
                            )
...
async def fetch_prices_infinite(tickers, batch_size=150):
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        while True:
            if missed_stocks:
                tasks = [
                    fetch_last_price_async(ticker, session) for ticker in missed_stocks
                ]
            ...
            for i in range(0, len(tickers), batch_size):
                ...
                tasks = [
                    fetch_last_price_async(ticker, session)
                    for ticker in batch
                    if ticker not in missed_stocks
                ]
                await asyncio.gather(*tasks)
                await asyncio.sleep(random.uniform(1, 5))
