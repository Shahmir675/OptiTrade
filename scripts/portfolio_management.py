...
def is_trading_time():
    ...
    holidays = cal.holidays(start=str(now_et.year) + "-01-01", end=str(now_et.year) + "-12-31").to_pydatetime()
...
def validate_limit_price(limit_price, current_price, order_type):
    ...
    lower_bound = (current_price * (1 - PRICE_TOLERANCE)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    upper_bound = (current_price * (1 + PRICE_TOLERANCE)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    
    if limit_price < MIN_TICK_SIZE:
        raise ValueError(f"Limit price {limit_price} is below the minimum tick size of {MIN_TICK_SIZE}.")
    if limit_price > ABSOLUTE_MAX_PRICE:
        raise ValueError(f"Limit price {limit_price} exceeds the maximum allowable price of {ABSOLUTE_MAX_PRICE}.")

    if order_type == "buy" and limit_price > current_price:
        raise ValueError(f"Buy limit price {limit_price} must be at or below the current price {current_price}.")
    if order_type == "sell" and limit_price < current_price:
        raise ValueError(f"Sell limit price {limit_price} must be at or above the current price {current_price}.")
    
    if not (lower_bound <= limit_price <= upper_bound) and order_type == 'buy':
        raise ValueError(f"Limit price {limit_price} is outside the acceptable range of {lower_bound} to {current_price}.")
    
    if not (lower_bound <= limit_price <= upper_bound) and order_type == 'sell':
        raise ValueError(f"Limit price {limit_price} is outside the acceptable range of {current_price} to {upper_bound}.")
...
def store_stop_loss_order(user_id, symbol, quantity, stop_price, order_type):
    ...
    if not triggered:
        try:
            with psycopg2.connect(**db_config) as conn:
                with conn.cursor() as cursor:
                    cursor.execute("SELECT COALESCE(MAX(order_id), 0) + 1 FROM stop_loss_orders")
                    order_id = cursor.fetchone()[0]
                    ...
                    cursor.execute(insert_query, (
                        order_id, user_id, symbol, order_type, stop_price, quantity, datetime.now()
                    ))
                    conn.commit()
            ...
            for order in pending_orders:
                ...
                try:
                    if order_type == 'sell' and current_price <= stop_price:
                        sell_stock(user_id, symbol, remaining_quantity, order_type='market')
                        cursor.execute("""
                        ...
                        buy_stock(user_id, symbol, remaining_quantity, order_type='market')
                        cursor.execute("""
                            UPDATE stop_loss_orders 
                            SET order_status = TRUE, 
                                filled_quantity = remaining_quantity,
...
def store_take_profit_order(user_id, symbol, quantity, take_profit_price, order_type):
    ...
    if not triggered:
        try:
            with psycopg2.connect(**db_config) as conn:
                with conn.cursor() as cursor:
                    cursor.execute("SELECT COALESCE(MAX(order_id), 0) + 1 FROM take_profit_orders")
                    order_id = cursor.fetchone()[0]
                    ...
                    """
                    cursor.execute(insert_query, (
                        order_id, user_id, symbol, order_type, take_profit_price, quantity, datetime.now()
                    ))
                    conn.commit()
...
def buy_stock(user_id, symbol, quantity, order_type='market', limit_price=None):
    with psycopg2.connect(**db_config) as conn:
        ...
        with conn.cursor() as cursor:
            ...
            u_balance = Decimal(u_balance).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
            ...
            if order_type == 'market':
                ...
                total_cost = (Decimal(quantity) * current_price).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                ...
                if portfolio_row:
                    ...
                    new_avg_price = ((portfolio_quantity * portfolio_avg_price + total_cost) / new_quantity).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                    new_total_invested = (portfolio_total_invested + total_cost).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                    ...
                    """
                    cursor.execute(update_portfolio_query, (new_quantity, new_avg_price, new_quantity * current_price, new_total_invested, user_id, symbol))
                else:
                    insert_portfolio_query = """
                        INSERT INTO portfolio (user_id, symbol, quantity, average_price, current_value, total_invested)
                        VALUES (%s, %s, %s, %s, %s, %s);
                    """
                    cursor.execute(insert_portfolio_query, (user_id, symbol, quantity, current_price, quantity * current_price, total_cost))

                new_cash_balance = (u_balance - total_cost).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                ...
                new_portfolio_value = (Decimal(u_portfolio) + quantity * current_price).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                new_net_worth = (new_cash_balance + new_portfolio_value).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                ...
                update_balance_query = """
                    ...
                    WHERE user_id = %s;
                """
                cursor.execute(update_balance_query, (new_portfolio_value, new_net_worth, user_id))
                cursor.execute("""
                    ...
                    """, (user_id, symbol, quantity, order_type, limit_price, current_price, total_cost))
                ...
                if current_price <= limit_price:
                    ...
                    store_limit_order(user_id, symbol, quantity, limit_price, order_type='buy')
                    return f"Limit order placed: Buy {quantity} shares of {symbol} at or below {limit_price} per share."
    ...
    with psycopg2.connect(**db_config) as conn:
        ...
        with conn.cursor() as cursor:
            ...
            if order_type == 'market':
                ...
                total_sale = (Decimal(quantity) * current_price).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)

                new_quantity = portfolio_quantity - quantity
                new_total_invested = (new_quantity * portfolio_avg_price).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                new_current_value = (new_quantity * current_price).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                ...
                if new_quantity > 0:
                    update_portfolio_query = """
                        ...
                        WHERE user_id = %s AND symbol = %s;
                    """
                    cursor.execute(update_portfolio_query, (new_quantity, new_current_value, new_total_invested, user_id, symbol))
                else:
                ...
                new_cash_balance = (Decimal(u_balance) + total_sale).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                update_balance_query = """
                    UPDATE user_balance
                    SET cash_balance = %s
                    WHERE user_id = %s;
                ...
                """, (user_id, symbol, quantity, order_type, None, total_sale, current_price))
                ...
                if current_price >= limit_price:
                    ...
                    """, (user_id, symbol, quantity, order_type, limit_price, total_sale, current_price))

                    return f"Limit order executed: {quantity} shares of {symbol} sold at {current_price} per share."
                else:
                    store_limit_order(user_id, symbol, quantity, limit_price, order_type='sell')
                    return f"Limit order placed: Sell {quantity} shares of {symbol} at or above {limit_price} per share."
