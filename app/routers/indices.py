...
async def nasdaq_summary():
    try:
        ...
        previous_close = await asyncio.to_thread(lambda: round(ticker.info.get('previousClose', 0), 2))
...
async def nasdaq_intraday():
    try:
        ticker = yf.Ticker('^IXIC')
        intraday_df = await asyncio.to_thread(lambda: ticker.history(period="1d", interval="1h"))
