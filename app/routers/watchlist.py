...
async def add_to_watchlist(
    user_id: int, stock_symbol: str, db: AsyncSession = Depends(get_db)
):
    ...
    if existing_entry:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Stock already in watchlist"
        )

    if not get_stock_info_from_json(stock_symbol):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Stock symbol not found"
        )

    new_watchlist_item = sql_models.Watchlist(
...
async def remove_from_watchlist(
    ...
    if not watchlist_item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Stock not in watchlist"
        )
