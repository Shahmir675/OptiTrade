...
from app.services import security_service, email_service
...
async def signup(request: pyd_models.SignupRequest, db: AsyncSession = Depends(get_db)):
    ...
    query = select(sql_models.UserModel).where(sql_models.UserModel.email == request.email)
    result = await db.execute(query)
    if result.scalars().first():
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Email already in use.")

    user_data = {"u_name": request.u_name, "email": request.email, "u_pass": request.u_pass}
...
async def verify_otp_route(request: pyd_models.OTPVerifyRequest, db: AsyncSession = Depends(get_db)):
    is_valid, message, user_data_from_otp = security_service.verify_otp(request.email, request.otp)
    ...
    new_user = sql_models.UserModel(u_name=user_data_from_otp["u_name"], email=user_data_from_otp["email"], u_pass=hashed_password)
    db.add(new_user)
    await db.commit()
...
async def forgot_password(request: pyd_models.ForgotPasswordRequest, db: AsyncSession = Depends(get_db)):
    query = select(sql_models.UserModel).where(sql_models.UserModel.email == request.email)
    result = await db.execute(query)
...
async def verify_reset_otp(request: pyd_models.ResetPasswordRequest, db: AsyncSession = Depends(get_db)):
    is_valid, message, _ = security_service.verify_otp(request.email, request.otp)
    if not is_valid:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=message)

    user_result = await db.execute(select(sql_models.UserModel).where(sql_models.UserModel.email == request.email))
