...
async def get_stock_historical_data(stock_symbol: str, start_date: str = None, end_date: str = None):
    data = await data_service.get_yfinance_stock_data_async(stock_symbol, start_date, end_date)

    if data is None or data.empty:
        stock_info = data_service.get_stock_info_from_json(stock_symbol)
        if stock_info:
            return JSONResponse(content=stock_info)
        raise HTTPException(status_code=404, detail="Stock data not found from yfinance or local JSON.")
    ...
    if not existing_cols: 
        raise HTTPException(status_code=404, detail="Stock data found but not in expected format.")
